  ...
  return (
    <div className="content-container fulfillment-detail-page">
      ...
      <section className="panel split-panel">
        ...
        <div className="split-total"><span>Estimated total shipping</span><strong>₹{order.stockRows.reduce((sum, r) => sum + Number(r.cost.replace(/[^0-9.-]+/g, '') || 0), 0).toLocaleString('en-IN')}</strong></div>
      </section>
      <div className="fulfillment-callout"><AlertTriangle size={16} /><span>{isReady ? 'Suggested split accepted and saved to database. Order is ready for release.' : 'AI-recommended multi-hub split across Mumbai, Bengaluru, and Delhi NCR fulfillment centres.'}</span></div>
      ...
      <div className="split-visual"><div className="split-warehouse"><span className="warehouse-icon"><Package size={17} /></span><div><strong>{order.stockRows[0]?.warehouse || 'Mumbai Hub'}</strong><small>{order.stockRows[0]?.quantity ?? '—'} · 1 shipment</small></div></div><ArrowRight size={18} /><div className="split-warehouse"><span className="warehouse-icon warehouse-east"><Package size={17} /></span><div><strong>{order.stockRows[1]?.warehouse || 'Bengaluru Hub'}</strong><small>{order.stockRows[1]?.quantity ?? '—'} · 1 shipment</small></div></div><div className="split-visual-line" /></div>
    </div>
  );
}
